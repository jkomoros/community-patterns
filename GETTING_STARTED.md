...
**Note:** This guide is for Claude Code to use when guiding new users through
workspace setup. Users should complete the pre-setup steps in SETUP.md first.
...
- ✅ Cloned their fork of community-patterns (includes `.mcp.json` for
  Playwright)
- ✅ Restarted Claude Code (MCP auto-configured from `.mcp.json`)

Now guide them through cloning dependencies, getting API keys, and creating
their workspace.
...
**Set these variables once at the beginning - you'll use them throughout
setup:**
...
**Tell the user:** "I've set up the working directories. Now let's clone the
dependencies."
...
Clone the labs repository (required) and patterns repository (optional, if
accessible) as peers to community-patterns.
...
5. **Enable required APIs** in Google Cloud Console:
   ...
   - **Drive API**:
     https://console.cloud.google.com/apis/api/drive.googleapis.com (required
     for Google Docs comments)
   - Docs API: https://console.cloud.google.com/apis/api/docs.googleapis.com
     (optional, for reading doc content)

Tell user: "You can skip the optional keys for now and add them later if
needed."
...
**Ask user for their Anthropic API key** and replace
`sk-ant-xxxx-their-actual-key-here` with it.
...
**Tell the user:** If both servers were already running, inform them: "Your dev
servers are already running, so I've skipped starting them."
...
**Tell the user:** "I'm going to open a browser so you can register. You'll need
to click 'Register', then 'Generate Passphrase', and paste it back to me so I
...
**Tell the user:** "I've copied a working counter pattern to your directory.
Let's deploy it to make sure everything is working."
...
`http://localhost:8000/test-$GITHUB_USER-1/CHARM-ID`
...
**Tell the user:** "Great! Your test pattern is working. The setup is now
complete. What would you like to build next?"
