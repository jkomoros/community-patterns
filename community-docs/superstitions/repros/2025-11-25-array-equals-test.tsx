  ...
  ({ items, lastResult }) => {
    ...
    >(
      (_, { items, targetItem, result }) => {
        ...
        // Test 1: === comparison
        const indexViaIdentity = currentItems.findIndex((item) =>
          item === targetItem
        );

        // Test 2: .equals() method
        const indexViaEquals = currentItems.findIndex((item) =>
          item.equals(targetItem)
        );
        ...
        result.set(
          `Testing "${targetName}":\n` +
            `=== comparison: index=${indexViaIdentity} (${
              indexViaIdentity >= 0 ? "FOUND" : "NOT FOUND"
            })\n` +
            `.equals() method: index=${indexViaEquals} (${
              indexViaEquals >= 0 ? "FOUND" : "NOT FOUND"
            })`,
        );
      },
    );
    ...
    >(
      (_, { items, targetItem, result }) => {
        ...
        if (index >= 0) {
          items.set(currentItems.toSpliced(index, 1));
          result.set(
            `Removed item at index ${index} using === (UNEXPECTED SUCCESS)`,
          );
        } else {
          result.set(
            `Could not find item using === (EXPECTED - superstition confirmed)`,
          );
        }
      },
    );
      ...
      [UI]: (
        <div style={{ padding: "20px", fontFamily: "system-ui" }}>
          ...
          >
            <h3>Claim</h3>
            <p>
              === comparison fails for Cells from .map(), must use .equals()
              instance method
            </p>
          </div>
          ...
          >
            ...
            <ul>
              <li>
                If === returns -1 but .equals() returns correct index →
                CONFIRMED
              </li>
