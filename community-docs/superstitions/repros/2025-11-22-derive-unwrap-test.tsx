...
export default pattern<Input, Output>(
  ({ flag, count }) => {
    ...
    const singleCellResult = derive(flag, (value) => {
      ...
      return `Single: type=${valueType}, hasGet=${hasGet}, value=${
        String(value)
      }`;
    });
    ...
    const objectResult = derive({ flag, count }, (values) => {
      ...
      const flagHasGet = values.flag && typeof values.flag === "object" &&
        "get" in values.flag;
      const countType = typeof values.count;
      const countHasGet = values.count && typeof values.count === "object" &&
        "get" in values.count;
      ...
      [UI]: (
        <div style={{ padding: "20px", fontFamily: "system-ui" }}>
          ...
          >
            ...
            <p>
              <strong>Result:</strong> {singleCellResult}
            </p>
          </div>
          ...
          >
            ...
            <p>
              Expected (if superstition false): hasGet=false (auto-unwrapped)
            </p>
            <p>
          ...
          >
            ...
            <ul>
              <li>
                If Test 2 shows hasGet=true → Superstition CONFIRMED (must use
                .get())
              </li>
              <li>
                If Test 2 shows hasGet=false → Superstition DISPROVED
                (auto-unwraps)
              </li>
            </ul>
          </div>
