...
export default pattern<Input, { [NAME]: string; [UI]: JSX.Element }>(
  ({ showInputA, showInputB, inputValueA, inputValueB, resultA, resultB }) => {
    ...
    const inputUIB = (
      <div
        style={{ background: "#e8f5e9", padding: "15px", borderRadius: "8px" }}
      >
        <p>
          <strong>Input B (pulled out):</strong>
        </p>
        <cf-input
          $value={inputValueB}
      ...
      [UI]: (
        <div style={{ padding: "20px", fontFamily: "system-ui" }}>
          ...
          >
            ...
            <ul>
              <li>
                <strong>Test A:</strong> UI inline in ifElse (reported broken)
              </li>
              <li>
                <strong>Test B:</strong> UI pulled out to variable first
              </li>
            </ul>
          </div>
          ...
          >
            ...
            <ul>
              <li>
                <strong>If A fails but B works:</strong>{" "}
                Transformer issue (inline JSX)
              </li>
              <li>
                <strong>If both fail:</strong> Deeper runtime issue with ifElse
              </li>
              <li>
                <strong>If both work:</strong>{" "}
                Bug may be fixed or context-specific
              </li>
            </ul>
          </div>
        </div>
