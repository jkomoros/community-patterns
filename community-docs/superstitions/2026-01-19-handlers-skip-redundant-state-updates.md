...
**Date:** 2026-01-19 **Status:** confirmed **Confidence:** high **Stars:** 5

## TL;DR - The Rule

**In handlers that fetch data and call `.set()`, always check if the data
actually changed before setting.** Otherwise, you may trigger unnecessary
reactive updates that cause infinite loops or 100% CPU usage.
...
**Symptom:** 100% CPU usage, frozen UI, or infinite loops when clicking UI
elements that trigger handlers.
...
- Handler runs → `.set()` fires → `computed()` re-runs → UI re-renders → new
  handler instances created

Even if the data hasn't changed, calling `.set()` with a new array/object
reference triggers all subscribers. This can cause:
...
```typescript
const changed = existing.length !== newItems.length ||
  newItems.some(
    (item, i) =>
...
The Common Fabric reactivity system tracks Cell subscriptions. When you call
`.set()`:
...
Even if the value is logically the same, a new array/object reference is
different from the old one, triggering the update cascade.
...
2. **The handler fetches data that rarely changes** (user profiles, config,
   calendar lists)
...
3. Add logging before `.set()` calls:
   `console.log("Setting calendars, changed:", calendarsChanged)`

## Key Takeaway

**Always guard `.set()` calls with a change check when the handler may run
multiple times.** This is especially important for handlers that fetch external
data and are bound to UI elements rendered in reactive contexts.
