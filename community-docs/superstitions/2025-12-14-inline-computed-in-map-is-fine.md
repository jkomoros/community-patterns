...
**Status:** Blessed (framework author confirmed 2025-12-14) **Source:**
Framework author direct guidance

## The Rule

Creating `computed()` inside `.map()` callbacks **SHOULD work** and is
**"actually the better style"** per framework author.
...
The framework auto-unwraps computed values in JSX context, so you don't need
`.get()`.
...
> "the reason [...] doesn't work is that it uses .get() to read from isRead.
> That should otherwise work and is actually the better style."

> "(The comment is also wrong, it doesn't recompute it on every render, it
> computes it whenever readUrls changes)"
...
- **RIGHT**: Inline computed() is fine and preferred, just don't call `.get()`
  on it
