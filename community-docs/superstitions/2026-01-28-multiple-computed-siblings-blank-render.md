...
// Result: ct-vstack renders with ZERO children. Blank page.
...
When a parent element (e.g., `ct-vstack`, `div`) has two or more `computed()` expressions as direct children, the entire parent renders empty — all children (including static ones) disappear from the DOM. This also applies to `.map()` as a sibling of `computed()`, which was discovered separately.
...
- **DOM structure exists but is empty** — e.g., `ct-vstack` element exists with zero children
...
| Test | Children of ct-vstack | Result |
|------|----------------------|--------|
...
<ct-vstack style="gap: 16px;">
  ...
  <div>
    {computed(() => {
      ...
      return <ct-input $value={email} placeholder="Email" />;
    })}
  </div>
  ...
  <div>
    {computed(() => {
      ...
      return <ct-input $value={phone} placeholder="Phone" />;
    })}
  </div>
</ct-vstack>
```
...
Note: `sectionHeader()` contains a nested `computed()` inside a `ct-hstack`. Since it's nested (not a direct child of the `<div>`), the `<div>` still has only one direct reactive child — the toggle `computed()`. The sectionHeader's `computed()` is a grandchild.
...
<ct-vstack>
  ...
  {computed(() => {
    const sections = [];
    if (showEmail.get()) sections.push(<ct-input $value={email} />);
    if (showPhone.get()) sections.push(<ct-input $value={phone} />);
    return <ct-vstack>{sections}</ct-vstack>;
  })}
</ct-vstack>
```
  ...
  [UI]: (
    <ct-screen>
      <ct-vstack style={{ gap: "16px", padding: "16px" }}>
        <ct-hstack>/* name inputs */</ct-hstack>
...
**Result:** `ct-vstack` rendered in DOM with zero children. Completely blank page.
