...
const LINK_EXTRACTION_SCHEMA = {
  ...
  properties: {
    urls: {
      ...
      description:
        "Security-related URLs found (CVEs, advisories, security blogs, GitHub security issues)",
    },
    classification: {
...
const CONTENT_CLASSIFICATION_SCHEMA = {
  ...
  properties: {
    isOriginalReport: {
      ...
      description:
        "TRUE if this IS an original security report (CVE, vendor advisory, researcher disclosure). FALSE if it's a news article/blog ABOUT a security issue.",
    },
    originalReportUrl: {
      type: "string" as const,
      description:
        "If isOriginalReport=false, the URL to the original report this article discusses. Null if isOriginalReport=true or no clear original found.",
    },
    confidence: {
      type: "string" as const,
...
const REPORT_SUMMARY_SCHEMA = {
  ...
  properties: {
    title: {
      type: "string" as const,
      description: "Clear name for the vulnerability/attack",
    },
    ...
    isLLMSpecific: {
      type: "boolean" as const,
      description:
        "TRUE if this is specifically about LLM/AI security (prompt injection, jailbreaking, etc.)",
    },
    discoveryDate: {
      type: "string" as const,
      description: "When reported (YYYY-MM or YYYY-MM-DD)",
    },
    canonicalId: {
      type: "string" as const,
      description:
        "Canonical identifier: CVE-XXXX-XXXXX if available, or a descriptive slug like 'log4shell-rce'",
    },
  },
  required: [
    "title",
...
const LINK_EXTRACTION_SYSTEM =
  `You are analyzing content to extract SECURITY-RELATED URLs only.
...
const REPORT_SUMMARY_SYSTEM =
  `Summarize this security report. Determine if it's LLM-specific:
...
// Pattern: Process a single URL slot through L2→L3→L4→L5
// Converted from a standalone helper to a pattern<> because the new cf
// compiler bans computed() inside standalone helper functions — reactive
// closures must live inside a pattern body.
type UrlSlotInput = { url: string | null };
type UrlSlotOutput = {
  sourceUrl: unknown;
  webContent: unknown;
...
const UrlSlotPattern = pattern<UrlSlotInput, UrlSlotOutput>(({ url }) => {
  ...
  const needsOriginalFetch = computed(() => {
    const cls = classification as {
      result?: { isOriginalReport: boolean; originalReportUrl?: string };
    } | null;
    return cls?.result && !cls.result.isOriginalReport &&
      isValidUrl(cls.result.originalReportUrl);
  });
  const originalReportUrl = computed(() => {
    const cls = classification as
      | { result?: { originalReportUrl?: string } }
      | null;
  ...
  const isOriginal = computed(() => {
    const cls = classification as
      | { result?: { isOriginalReport: boolean } }
      | null;
...
// Note: processArticleUrls used to be a standalone helper function, but the
// new cf compiler bans computed() inside module-scope helpers. It is now
// inlined into the .map() callbacks inside the main pattern body.

const PromptInjectionTracker = pattern<TrackerInput, TrackerOutput>(
  ({ gmailFilterQuery, limit, articles }) => {
    // ==========================================================================
    // Gmail Integration
    // ==========================================================================
    // GmailImporter manages auth internally via createGoogleAuth()
    ...
    const articleExtractions = computed(() => [
      ...(Array.isArray(manualArticleExtractions)
        ? manualArticleExtractions
        : []),
      ...(Array.isArray(emailArticleExtractions)
        ? emailArticleExtractions
        : []),
    ]);
    ...
    const _debugL1CellStructure = computed(() => {
          ...
          return {
            ...
            extraction: ext
              ? {
                ...
                resultType: ext.result === undefined
                  ? "undefined"
                  : ext.result === null
                  ? "null"
                  : typeof ext.result,
      ...
      console.log(
        "[DEBUG:L1-CELL-STRUCTURE]",
        JSON.stringify({ totalItems: list.length, sample }, null, 2),
      );
      return sample;
    });
    ...
    const _classificationCounts = computed(() =>
      ...
      (articleExtractions as any[])
        ...
        .reduce(
          ...
          {
            "has-security-links": 0,
            "is-original-report": 0,
            "no-security-links": 0,
          },
        )
    );
    ...
    const manualUrlProcessing = manualArticleExtractions.map((article) => {
      // Inlined from processArticleUrls: the new cf compiler bans
      // computed()/pattern instantiation inside standalone helper functions.
      const url0 = computed(() => {
      ...
      return {
        ...
        slots: [
          UrlSlotPattern({ url: url0 }),
          UrlSlotPattern({ url: url1 }),
          UrlSlotPattern({ url: url2 }),
        ],
      };
    });
          ...
          return {
            ...
            webContent: wc
              ? {
                ...
                resultType: wc.result === undefined
                  ? "undefined"
                  : wc.result === null
                  ? "null"
                  : typeof wc.result,
    ...
    const _debugL2Counts = computed(() => {
      const counts = {
        pending: fetchPendingCount,
        success: fetchSuccessCount,
        error: fetchErrorCount,
        done: fetchCompletedCount,
      };
      if (DEBUG_LOGGING) {
          ...
          return {
            ...
            classification: cl
              ? {
                ...
                resultType: cl.result === undefined
                  ? "undefined"
                  : cl.result === null
                  ? "null"
                  : typeof cl.result,
                hasIsOriginalReport:
                  !!cl.result?.isOriginalReport !== undefined,
    ...
    const originalReportUrls = computed(() => {
      ...
      const seen = new Set<string>();
      const urls: Array<
        { url: string; sourceUrl: string; isDirectOriginal: boolean }
      > = [];
    ...
    const l4CompletedCount = computed(() =>
      ...
      (contentClassifications as any[]).filter((item: any) => {
        ...
        if (
          isValidUrl(result.originalReportUrl) && item.originalContent?.result
        ) return true;
        return false;
      }).length
    );
      ...
      [UI]: (
        ...
        >
          ...
          <p style={{ fontSize: "12px", color: "#666", marginBottom: "16px" }}>
            Extracts URLs from security newsletters. Uses Gmail integration +
            LLM extraction.
          </p>
          ...
          <details style={{ marginBottom: "16px" }}>
            ...
            <div style={{ padding: "12px", marginTop: "8px" }}>
              ...
              <p style={{ fontSize: "11px", color: "#666", marginTop: "8px" }}>
                Loads sample security newsletter content for testing without
                Gmail. Use "+ Add 1 Article" to test incremental caching.
              </p>
            </div>
          </details>
          ...
          >
            ...
            >
              ...
              <div
                style={{
                  padding: "6px 10px",
                  background: fetchPendingCount > 0
                    ? "#fef3c7"
                    : fetchErrorCount > 0
                    ? "#fee2e2"
                    : "#dbeafe",
                ...
                <div style={{ fontSize: "14px", fontWeight: "bold" }}>
                  ...
                  {fetchErrorCount > 0 && (
                    <span
                      ...
                      title={`${fetchErrorCount} failed - will retry`}
                    >
                      ⚠️{fetchErrorCount}
                    </span>
                  )}
              ...
              <div
                style={{
                  ...
                  borderRadius: "6px",
                  border: `1px solid ${
                    classifyPendingCount > 0 ? "#fcd34d" : "#a5b4fc"
                  }`,
                }}
              >
                <div style={{ fontSize: "10px", color: "#666" }}>
              ...
              <div
                style={{
                  ...
                  borderRadius: "6px",
                  border: `1px solid ${
                    l4PendingCount > 0 ? "#fcd34d" : "#6ee7b7"
                  }`,
                }}
              >
              ...
              <div
                style={{
                  ...
                  borderRadius: "6px",
                  border: `1px solid ${
                    summaryPendingCount > 0 ? "#fcd34d" : "#c4b5fd"
                  }`,
                }}
              >
                <div style={{ fontSize: "10px", color: "#666" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {articleExtractions.map((item) => (
              <div
                style={{
                  padding: "12px",
                  background: item.extraction.pending
                    ? "#fef3c7"
                    : item.extraction.result?.classification ===
                        "is-original-report"
                    ? "#dbeafe"
                    : item.extraction.result?.classification ===
                        "has-security-links"
              ...
              >
                ...
                >
                  ...
                  {!item.extraction.pending && (
                    <span
                      style={{
                        ...
                        padding: "2px 6px",
                        borderRadius: "4px",
                        background: item.extraction.result?.classification ===
                            "is-original-report"
                          ? "#3b82f6"
                          : item.extraction.result?.classification ===
                              "has-security-links"
                  ...
                  <span style={{ color: "#666", fontSize: "12px" }}>
                    {item.extraction.pending
                      ? "processing..."
                      : `${item.extraction.result?.urls?.length ?? 0} links`}
                  </span>
                </div>
                {!item.extraction.pending &&
                  (item.extraction.result?.urls?.length ?? 0) > 0 && (
                  <ul
                    style={{
                      margin: "4px 0 0 16px",
                      padding: 0,
          ...
          {uniqueOriginalCount > 0 && (
            <div style={{ marginTop: "24px" }}>
              ...
              >
                ...
                {summaryPendingCount > 0 && (
                  <span style={{ color: "#f59e0b", fontSize: "14px" }}>
                    ⏳ {summaryPendingCount} processing...
                  </span>
                )}
              </h3>
              <div
              ...
              >
                Deduplicated from {linkCount} source URLs. 🤖 {llmSpecificCount}
                {" "}
                LLM-specific.
              </div>
              <div
              ...
              >
                {finalReportsWithReadState.map((item) => (
                  <div
                    style={{
                      padding: "12px",
                      background: item.summary?.pending
                        ? "#fef3c7"
                        : item.summary?.result?.isLLMSpecific
                        ? "#fce7f3"
                        : item.summary?.result?.severity === "critical"
                        ? "#fee2e2"
                        : item.summary?.result?.severity === "high"
                        ? "#ffedd5"
                        : "#f0fdf4",
                      borderRadius: "6px",
                      border: `1px solid ${
                  ...
                  >
                    ...
                    >
                      ...
                      {item.summary?.pending ? <span>⏳ Analyzing...</span> : (
                        <>
                          <span
                            style={{ fontWeight: item.isRead ? "400" : "600" }}
                          >
                            {item.summary?.result?.title || "Unknown"}
                          </span>
                            ...
                            style={{
                              ...
                              background:
                                item.summary?.result?.severity === "critical"
                                  ...
                                  : item.summary?.result?.severity === "high"
                                  ? "#ea580c"
                                  : item.summary?.result?.severity === "medium"
                                  ? "#ca8a04"
                                  : "#16a34a",
                              color: "white",
                            }}
