...
>((_, { ruleSuggestionPrompt, event }) => {
  ...
  const prompt =
    `Event was manually classified as "${event.category}" for family "${event.familyName}".
...
>((_, { hostingEvents, manualEventForm, ruleSuggestionPrompt }) => {
  ...
  const prompt =
    `Event was manually classified as "${form.category}" for family "${form.familyName}".
...
function classifyEventWithRules(
  ...
  myAddresses: Address[],
  familyAddresses: Record<string, Address[]>,
  ...
  for (const [familyId, addresses] of Object.entries(familyAddresses)) {
...
function ruleMatches(
  ...
  switch (rule.type) {
    ...
    case "location_exact":
      return event.location.toLowerCase().trim() ===
...
const HostingTracker = pattern<HostingTrackerInput>(
  ({
    ...
    // Build family addresses map. Uses Record<> instead of Map<> because the
    // new SES verifier rejects non-JSON-serializable Map values in pattern
    // inputs/outputs.
    const familyAddressesMap = computed(() => {
      ...
      for (const family of trackedFamilies) {
        if (family.addresses.length > 0) {
          map[family.id] = family.addresses;
        }
      }
      return map;
    });
    ...
    const recentEvents = computed(() =>
      [...hostingEvents].sort((a, b) => b.date.localeCompare(a.date)).slice(
        0,
        10,
      )
    );
    ...
    const googleCalendarCharm = wish<{ events: GoogleCalendarEvent[] }>({
      query: "#googleCalendar",
    });
    const appleCalendarCharm = wish<{ events: AppleCalendarEvent[] }>({
      query: "#appleCalendar",
    });
    ...
    const hasGoogleCalendar = computed(() =>
      googleCalendarCharm?.result?.events &&
      googleCalendarCharm.result.events.length > 0
    );
    const hasAppleCalendar = computed(() =>
      appleCalendarCharm?.result?.events &&
      appleCalendarCharm.result.events.length > 0
    );
    ...
    const llmSuggestions = generateObject<RuleSuggestionResponse>({
      ...
      system:
        `You are a classification rule expert for a family hosting tracker app.
    ...
    return {
      ...
      [UI]: (
        <cf-screen>
          ...
          <cf-autolayout
            tabNames={["Dashboard", "Families", "Events", "Rules"]}
          >
            {/* ========== TAB 1: DASHBOARD ========== */}
              ...
              <cf-vstack style="padding: 16px; gap: 16px;">
                ...
                <cf-vstack style="gap: 8px;">
                  ...
                  >
                    ...
                    Create charms for families and calendars, then favorite them
                    (star icon) with the appropriate hashtag. This tracker
                      ...
                      {overdueStats.map((stat) => (
                        ...
                        >
                          <div>
                            ...
                            <div style={{ fontSize: "12px", color: "#666" }}>
                              Last hosted us: {stat.daysSinceTheyHosted !== null
            ...
            <cf-vscroll flex showScrollbar>
              <cf-vstack style="padding: 16px; gap: 16px;">
                ...
                >
                  <strong>Tip:</strong>{" "}
                  Create a Family charm for each family you want to track. After
                  creating, favorite it (star icon) with the{" "}
                        ...
                        <div
                          style={{
                            ...
                            border: computed(() =>
                              stat
                                ? `1px solid ${
                                  STATUS_COLORS[stat.status].border
                                }`
                                : "1px solid #e5e7eb"
                            ),
                          }}
                              ...
                              <div style={{ fontSize: "12px", color: "#666" }}>
                                {computed(() =>
                                  family.primaryAddress
                                    ? family.primaryAddress.fullAddress
                                    : "No address"
                                )}
                              </div>
                            </div>
                                    ...
                                    <div>
                                      <div
                                        style={{
                                          fontWeight: "bold",
                                          color:
                                            STATUS_COLORS[stat.status].text,
                                        }}
                                      >
                  ...
                  >
                    <cf-vstack style="gap: 8px;">
                      ...
                      <cf-hstack style="gap: 8px;">
                        ...
                        <cf-button
                          variant="secondary"
                          onClick={addEventAndSuggestRules({
                            hostingEvents,
                            manualEventForm,
                    ...
                    <cf-vstack style="gap: 4px;">
                      {rules.map((rule) => (
                        ...
                        >
                          ...
                          <span
                            style={{ flex: 1, color: "#666", fontSize: "12px" }}
                          >
                          ...
                          <span
                            style={{
                              padding: "2px 8px",
                              backgroundColor: computed(() =>
                                CATEGORY_COLORS[rule.category].bg
                              ),
                              color: computed(() =>
                                CATEGORY_COLORS[rule.category].text
                              ),
                              borderRadius: "4px",
                              fontSize: "11px",
                            }}
                    ...
                    <cf-vstack style="gap: 8px;">
                      <cf-hstack style="gap: 8px;">
                        ...
                        <label style={{ flex: 1 }}>
                          ...
                          <select
                            value={computed(() =>
                              newRuleForm.get().type || "location_exact"
                            )}
                          ...
                          >
                            <option value="location_exact">
                              Location (exact)
                            </option>
                            <option value="location_regex">
                              Location (regex)
                            </option>
                      ...
                      <cf-hstack style="gap: 8px;">
                        <label style={{ flex: 1 }}>
                          ...
                          <select
                            value={computed(() =>
                              newRuleForm.get().category || "they-hosted"
                            )}
                              ...
                              >
                                <cf-hstack style="justify-content: space-between; align-items: flex-start;">
                                  <cf-vstack style="gap: 4px; flex: 1;">
                                    ...
                                    {ifElse(
                                      computed(() =>
                                        suggestion.potentialFalsePositives &&
                                        suggestion.potentialFalsePositives
                                            .length > 0
                                      ),
