...
**Status:** Draft for Framework Author Review **Date:** 2025-12-15 **Author:**
jkomoros + Claude
...
This document proposes an idiomatic architecture for rebuilding the
extracurricular-selector pattern from scratch. The current implementation (v1)
suffers from ConflictErrors and uses React-style patterns that don't fit the
framework's design philosophy.
...
A pattern to help parents select compatible extracurricular activities for their
child from multiple sources (afterschool programs, external classes).

### Core Features

1. **LLM Import** - Paste messy HTML/text, extract structured class data with
   eligibility triage
...
5. **Status Tracking** - Track registration progress (Registered → Confirmed →
   Paid → On-Calendar)
...
The v1 pattern gets CAS (compare-and-swap) conflicts immediately on load, before
any user interaction. Likely caused by:
...
> "In our system you'd want to not have local IDs and accumulate state per
> object."
...
interface ExtracurricularInput {
  ...
  categoryTags: Default<
    string[],
    ["Robotics", "Dance", "Art", "Music", "Sports"]
  >;
...
const togglePin = (cls: Class) => {
  ...
  if (sets.includes(activeSetName)) {
    ...
    if (idx >= 0) {
      classes.set(
        classes.get().toSpliced(idx, 1, { ...cls, pinnedInSets: updated }),
      );
    }
  } else {
    // Pin - add to set
    ...
    if (idx >= 0) {
      classes.set(
        classes.get().toSpliced(idx, 1, {
          ...cls,
          pinnedInSets: [...sets, activeSetName],
        }),
      );
    }
  }
    ...
    for (const slotB of b.timeSlots) {
      ...
      if (!Cell.equals(a.location, b.location)) {
        const travel = travelTimes.find((t) =>
          (Cell.equals(t.fromLocation, a.location) &&
            Cell.equals(t.toLocation, b.location)) ||
          (Cell.equals(t.fromLocation, b.location) &&
            Cell.equals(t.toLocation, a.location))
        );
...
> "Let's make sure to add guidance to box active/selected/etc selections. So
> write into something of type `Cell<{ selected: Cell<...> }>`. Then this will
> be `active.selected` or so. This avoids the `Cell<Cell<...>>` stuff that is
> brittle."
