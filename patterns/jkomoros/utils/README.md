...
This directory contains custom utilities created for jkomoros patterns (not from
upstream labs).
...
Word-level diff computation and field comparison utilities for LLM extraction
preview.

**Used by:**

- `person.tsx` - Shows field-by-field diffs when extracting person data from
  notes
- `food-recipe.tsx` - Shows diffs when extracting recipe data from notes

**API:**

- `computeWordDiff(from: string, to: string): DiffChunk[]` - Compute word-level
  diff between two strings
- `compareFields<T>(extracted: Partial<T>, fieldMappings): Array<{ field, from, to }>` -
